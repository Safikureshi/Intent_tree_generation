...
def print_cluster_tree(clusters, roots):
    def print_node(node_id, level=0):
        ...
        print(f"{indent}Cluster[{node_id}] (Level: {node['hierarchical_level']}): {generate_intent_label(node['questions'])}")
        for child_id in node.get('children', []):
            print_node(child_id, level + 1)

    for root in roots:
        print(f"Root Cluster[{root['id']}] (Level: {root['hierarchical_level']}): {generate_intent_label(root['questions'])}")
...
def store_embeddings(treeid, embedding):
    query = """
    ...
    try:
        for embedding in embedding:
            cur.execute(query, (treeid, embedding))  # Removed .tolist()
        conn.commit()
