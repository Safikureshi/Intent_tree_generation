...
def insert_question(question, cluster_ids, all_clusters, similarity_threshold):
    ...
    if matched_clusters:
        ...
        for cluster in matched_clusters:
            ...
            new_centroid = np.mean([cluster['centroid'], question_embedding], axis=0)
            cluster['centroid'] = new_centroid.tolist()
            try:
                cur.execute("UPDATE intent_embeddings SET embedding = %s WHERE treeid = %s", (json.dumps(new_centroid.tolist()), cluster['id']))
                logging.debug(f"Updated centroid for cluster {cluster['id']}")
            except Exception as e:
                logging.error(f"Error updating centroid for cluster {cluster['id']}: {e}")
        conn.commit()
        print(f"Added question to existing Clusters: {[cluster['id'] for cluster in matched_clusters]}")
    else:
        new_cluster = {
            'id': len(all_clusters),
            'parent_id': None,
        ...
        try:
            cur.execute("INSERT INTO intent_embeddings (treeid, embedding) VALUES (%s, %s)", (new_cluster['id'], json.dumps(new_cluster['centroid'])))
...
def update_intent_tree(treeid, new_question, all_clusters, similarity_threshold):
    print(new_question)
    query = """UPDATE intent_tree SET questions = questions || %s WHERE treeid = %s"""
    cur.execute(query, ("{" + ",".join(new_question) + "}", treeid))
    try:
        ...
        if not result:
            ...
            return None
        logging.debug(f"Found treeid: {treeid} in intent_embeddings")
        cluster_id = result[0]
        # Update the intent tree with the new question
        cur.execute(query, (new_question, treeid))
        if cluster_id is not None:
            insert_question(new_question, [cluster_id], all_clusters, similarity_threshold)
        conn.commit()
        logging.debug(f"Updated intent tree with treeid: {treeid}")
    except Exception as e:
        logging.error(f"Error updating intent tree for treeid {treeid}: {e}")
        conn.rollback()
...
def main():
    ...
    user_input = input("Enter the questions (comma-separated): ")
    if user_input is not None:
        new_questions = [q.strip() for q in user_input.split(',')]
        print(new_questions)
    all_clusters = [...]  # Load or define the all_clusters list
    similarity_threshold = 0.7  # Define the similarity threshold
    result = update_intent_tree(tree_id, new_questions, all_clusters, similarity_threshold)
    print(result)
